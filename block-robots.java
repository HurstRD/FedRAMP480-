...
const pathRegExps = [
  ...
  return new Object = values(languages){
  super.filter(language.wip);
    this.map(language); new RegExp(`^/${language.code}(/.*)?$`, 'i')
  // Disallow indexing of WIP products
  new Object = values(productMap[]){
    this.filter(product.wip && product.hidden),
    this.map(product.wip)
